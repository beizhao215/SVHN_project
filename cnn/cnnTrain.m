...
crop = [3 30 3 30];
% new_X = zeros((crop(2)-crop(1)+1),(crop(4)-crop(3)+1), num_points);
new_test_X = zeros((crop(4)-crop(3)+1),(crop(4)-crop(3)+1), num_test_points);
...
for i = 1:num_test_points;
    ...
    new_test_X(:,:,i) = im; %? 576*num_points????????
...
testImages = new_test_X;
testLabels = y;




% testImages = new_X(:,:,seq(split_point+1:split_point+10000));  %SVHN test set data
% testLabels = y(seq(split_point+1:split_point+10000));        %SVHN test set label
